...
function useActivityTypeParam(): ActivityType {
    ...
    const activityType = ActivityTypeValidator.safeParse(
        params.get('activity-type'),
    );
...
const ActivityCardsLoader = () =>
    Array(4)
        .fill(null)
        .map((_, i) => {
            return <Card className={'bg-border animate-pulse h-16'} key={i} />;
        });
...
function getPaddedClippedContent(
...
export default function ActivityLog() {
    ...
    function activityLogForSelectedTab() {
        ...
        switch (activityLog.data.activity_type) {
            case 'comments':
                return activityLog.data.comments.map((comment) => (
                    <Link
                        key={comment.id}
                        href={`/petitions/${comment.petition_id}`}>
                        <ActivityCard>
                            Commented
                            <Bold>
                            ...
                            on দাবি:
                            <Bold>
                                {getPaddedClippedContent(
                                    comment.petition_title,
                                )}
                            </Bold>
                        </ActivityCard>
                    </Link>
            ...
            case 'comment_votes':
                return activityLog.data.comment_votes.map((commentVote) => (
                    <Link
                        key={commentVote.id}
                        href={`/petitions/${commentVote.petition_id}`}>
                        <ActivityCard>
                            <Bold>
                                {' '}
                                {commentVote.vote > 0
            ...
            case 'petitions':
                return activityLog.data.petitions.map((petition) => (
                    <Link key={petition.id} href={`/petitions/${petition.id}`}>
                        <ActivityCard>
                            Created a দাবি:
                            <Bold>
                                {getPaddedClippedContent(petition.title)}
                            </Bold>
                        </ActivityCard>
                    </Link>
                ));
            case 'petition_votes':
                return activityLog.data.petition_votes.map((vote) => (
                    <Link key={vote.id} href={`/petitions/${vote.petition_id}`}>
                        <ActivityCard>
                            <Bold>
                                {' '}
                                {vote.vote > 0 ? 'Liked' : 'Disliked'}{' '}
                            </Bold>
                            a দাবি:
                        ...
                        <PaginationContent>
                            {activityLog?.data.page !== 0 ? (
                                <PaginationItem>
                                    <PaginationPrevious
                                        onClick={() =>
                                            getPageOfCurrentActivityType(
                                                activityLog.data.page - 1,
                                            )
                                        }
                                    />
                                </PaginationItem>
