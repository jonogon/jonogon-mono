...
export default function SimilarPetitionsSuggestions({
    title,
    onClose,
}: SimilarPetitionsSuggestionsProps) {
    const [similarPetitions, setSimilarPetitions] = useState<SimilarPetition[]>(
        [],
    );

    const {data: suggestionResults, refetch} =
        trpc.petitions.suggestSimilar.useQuery({title}, {enabled: false});
            ...
            <CardHeader className="flex flex-row items-center space-x-2 py-2">
                <div className="flex items-center">
                    ...
                    <CardTitle className="text-lg font-bold text-red-500">
                        Similar Petitions Found
                    </CardTitle>
                </div>
            </CardHeader>
            <CardContent className="py-2">
                <p className="mb-2 text-xs text-stone-600 font-medium">
                    We found some petitions that might be similar to yours.
                    Please check if your petition already exists:
                </p>
                    ...
                    {displayedPetitions.map((petition) => (
                        <li key={petition.id} className="py-1">
                            <Link
                                ...
                                className="text-neutral-900 flex items-center justify-between hover:bg-gray-50 rounded p-1 transition-colors duration-200"
                                target="_blank"
                ...
                {hasMorePetitions && (
                    <p className="mt-1 text-xs text-stone-500 italic">
                        And {similarPetitions.length - 3} more similar
                        petitions...
                    </p>
                )}
                <Button
