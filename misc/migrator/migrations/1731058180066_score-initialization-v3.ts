...
export async function up(pgm: MigrationBuilder): Promise<void> {
  ...
  for (const petition of petitions) {
    ...
    const logScore = activityScore >= VOTE_THRESHOLD ? Math.log(finalScore) + diff : Math.log(finalScore)
    await pgm.db.query(`UPDATE petitions SET score = $1,log_score = $2 WHERE id = $3`, [
      activityScore,
      logScore,
      petitionId,
    ]);
